...
module Test.Check.Discover.Internal.Config
...
parseConfig srcDir prog args = case getOpt' Permute (options srcDir) args of
  (opts, rest, rest', []) ->
    let config = foldl (flip id) (defaultConfig srcDir) { tastyOptions = rest ++ rest' } opts in
      if config.noModuleSuffix || isJust config.moduleSuffix
        then error moduleSuffixDeprecationMessage
        else if not $ null config.ignoredModules
...
options srcDir =
  ...
  , Option [] ["ignore-module"]
      (ReqArg (\s c -> c {ignoredModules = s : c.ignoredModules}) "FILE")
      "<<<DEPRECATED>>>: Ignore a test module"
  , Option [] ["ingredient"]
      (ReqArg (\s c -> c {tastyIngredients = s : c.tastyIngredients}) "INGREDIENT")
